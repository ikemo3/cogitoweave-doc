...
2. FOUNDATION マーカー（結論）
   - FOUNDATION は「文書 → 基底文書」へ結論を射影する射に相当。
   - 圏論的には関手的な構造保存射に近い。
...
4. CONCLUSION マーカー（結論）
   - CONCLUSION は文書内での完全な結論定義を行うが、基底文書への射影は実行しない射。
...
- 基底文書（foundation）: Why/What の統合結果は、対応する図式の（共）極限
...
### マーカー仕様（FOUNDATION / CONCLUSION / PREMISE / REFERENCE）
